...
@interface ImageDtailCell ()<UIScrollViewDelegate>
{
    //ScrollView
...
-(void)setImageUrl:(NSString *)imageUrl{
    ...
    [_imageView sd_setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:nil options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self->_loading show];
            self->_loading.progress = (CGFloat)receivedSize/(CGFloat)expectedSize;
        });
...
-(void)showEnlargeAnimation{
    ...
    [UIView animateWithDuration:0.35 animations:^{
        self->_collectionView.backgroundColor = [UIColor colorWithWhite:0 alpha:1];
        self->_imageView.frame = [self imageViewFrame];
...
-(void)showShrinkDownAnimation{
    ...
    [UIView animateWithDuration:0.35 animations:^{
        self->_collectionView.backgroundColor = [UIColor colorWithWhite:0 alpha:0];
        if (self->_isStart && self->_imageView.image) {
            self->_imageView.frame = self->_anchorFrame;
            
        }else{self.alpha = 0;}
    }completion:^(BOOL finished) {
        //通知回调
        self->_finishHideBlock();
        self->_imageView.frame = [self imageViewFrame];
        self.alpha = 0;
        self->_scrollView.zoomScale = minZoomScale;
        [self->_scrollView addSubview:self->_imageView];
        [self->_scrollView setContentOffset:CGPointZero];
    }];
}
    ...
    if (pan.state == UIGestureRecognizerStateEnded) {
        if (ABS(_scrollView.contentOffset.y) < minPanLength) {
            ...
            [UIView animateWithDuration:0.35 animations:^{
                ...
                frame.origin.y = self->_scrollView.bounds.size.height;
